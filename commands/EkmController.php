...
class EkmController extends Controller
{
    ...
    protected function saveSummary($id, $report, $summary)
    {
        ...
        if (null === $meterSummary) {
            ...
            $meterSummary->start_date = date('Y-m-d H:i:s', strtotime($meterSummary->start_timestamp));
            $meterSummary->end_date = date('Y-m-d H:i:s', strtotime($meterSummary->end_timestamp));
